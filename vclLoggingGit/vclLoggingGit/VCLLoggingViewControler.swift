...
class VCLLoggingViewControler: UIViewController {
    ...
    private static func bumbInstanceCount (for logginName: String) -> Int {
        logGlobals.instanceCounts[logginName] = (logGlobals.instanceCounts[logginName] ?? 0) + 1
        return logGlobals.instanceCounts[logginName]!
    }
    ...
    private func logVCL (_ msg: String) {
        if instanceCount == nil {
            instanceCount = VCLLoggingViewControler.bumbInstanceCount(for: vclLoggingName)
        }
        print("\(VCLLoggingViewControler.logPrefix(for: vclLoggingName)) (\(instanceCount!))\(msg)")
    }
    
    required init?(coder aDecoder: NSCoder) {
        super.init(coder: aDecoder)
        logVCL("\tinit(coder:) - create via InterfaceBuldier\n")
    }
    ...
    override func viewWillTransition(to size: CGSize, with coordinator: UIViewControllerTransitionCoordinator) {
        ...
        coordinator.animate(alongsideTransition: { (contex:
            UIViewControllerTransitionCoordinatorContext!) -> Void in
            self.logVCL("begin animate(alongsideTransition:completion:)")
        }, completion: { contex -> Void in
            self.logVCL("end anmiate(alongsideTransition:completion)")
        })
    }
}
